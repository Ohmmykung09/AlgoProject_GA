...
public class GA {
    ...
    public static class Result {
        ...
        // [NEW] เก็บประวัติ Cost แต่ละ Gen
        public List<Integer> costHistory = new ArrayList<>();
    }
        ...
        public void calculateFitness() {
            ...
            for (int step = 0; step < stepLimit; step++) {
                ...
                for (int p = 0; p < 4; p++) {
                    ...
                    if (curR == goalPoint[0] && curC == goalPoint[1])
                        reachedGoal = true;
                    break;
                }
    ...
    public Result run() {
        ...
        Result result = new Result(); // [NEW] Create result early
        ...
        for (int gen = 0; gen < GENERATIONS; gen++) {
            ...
            if (callback != null) {
                ...
                callback.onUpdate(bestGlobal.path, gen + 1, displayCost, currentTimeElapsed, status, allDeadEnds,
                        isNewBest);
                try {
                    Thread.sleep(1);
    ...
    public interface VisualizationCallback {
        void onUpdate(List<int[]> path, int gen, int cost, double timeElapsed, String status, List<int[]> deadEnds,
                boolean isNewBest);
    }

    public void setCallback(VisualizationCallback cb) {
        this.callback = cb;
    }
}